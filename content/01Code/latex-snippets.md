...
首先我们需要知道snippets的基本格式是这样的。
```
{trigger: "触发文本", replacement: "替换文本", options: "选项"}
```
其中，替换文本中的`$0`、`$1`代表按下`tab`键后光标跳转的先后位置，一段语法中也可以有多个`$0`或者`$1`，此时代表两处同步进行编辑（如数学环境下的`beg`）。
参考[该教程](https://forum-zh.obsidian.md/t/topic/22521)，它提供的选项有
- `m` : math的意思，只在公式块中启用
- `t` : text的意思，只在公式块外启用
- `A` : Auto的意思，不需要按Tab就会执行替换
- `r` : Regex正则表达式， `trigger`的内容会被解析一下
